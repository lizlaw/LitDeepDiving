...
subtitle: "Slides & scripts:<br>https://github.com/lizlaw/LitDeepDiving"
...
Further scripts available in the [slide github folder](https://github.com/lizlaw/LitDeepDiving)
Data (bib and txt bibliographies from WoS) in [OSF folder](https://doi.org/10.17605/OSF.IO/3SDWC)
...
(thought it might be useful to mention e.f. what Keywordsplus is):
...
```{r echo=TRUE}
tag_keywords <- litsearchr::extract_terms(keywords = paste(cleaned_data$keywords, cleaned_data$keywords),
                                           method = "tagged",
                                           min_freq = 2,
                                           stopwords ="English")
```
...
```{r echo=TRUE}
rake_keywords <- litsearchr::extract_terms(text = paste(cleaned_data$title, cleaned_data$abstract),
                                           method = "fakerake",
                                           min_freq = 2,
                                           stopwords ="English")
```
...
Load as tab delimited files - note text cleaning is done automatically with no options documented (code easy to examine/modify)
...
## scimeetr example - futher functions
...
## revtools example - deduplicate
Liz to add

## revtools example - screen titles
Liz to add

## revtools example - screen abstracts
Liz to add
...
## Summary on grouping literature (litsearchr, scimeetr, bibliometrix)

Benefits:

- Groups that can be used to:
    - explore large searches
    - alternative method to narrow down searches

Caveats: 

- emerging methods may need further development, e.g. further 'cleaning' and customising:
     - Year is missing for many interesting early access articles
     - may need 'stemming' of keywords (e.g. in scimeetr)
     - may need to include'synonyms' and n-grams (e.g. in scimeetr)
...
- allows screening of more than just title/abstract/keywords
- still requires (many more) subjective decisions
- attention to:
   - quality data in ~ reasonable data out
...
However, methods constantly improving, and we can improve the data going in through standardisation, term lists, etc...

---

It is so easy to automatically collate a large database of potential papers
![](https://media.giphy.com/media/VcizxCUIgaKpa/giphy.gif)
But we need to be pretty clever about then excluding those that are less relevant
